...
3. [Provide the correct variant](#provide-the-correct-variant)
4. [Run the `yarf` command](#run-the-yarf-command)
...
- Zapper: A hardware test fixture enabling advanced system testing. For details please visit [here](https://canonical-zapper.readthedocs-hosted.com/en/latest/)
...
If the `--platform` argument is not specified, then YARF will use Zapper as the platform:
```
ZAPPER_IP=<ZAPPER_IP> yarf <path-to-suite>/suite
```
