...
We can enter the interactive console by supplying an empty suite path. For example, in terms of Zapper, we can use:

```shell
ZAPPER_IP=<ip> yarf
```

<u><center>Code Snippet 1: Zapper's `yarf` command that enters interactive console</center></u>
