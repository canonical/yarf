...
def vnc_hid():
    hid = Hid()
    hid.vnc.safe_connect = MagicMock()
    return hid
...
class TestVncHid:
    ...
    async def test_keys_combo(self, monkeypatch, vnc_hid):
        with monkeypatch.context() as m:
            ...
            client_mock = (
                vnc_hid.vnc.safe_connect.return_value.__aenter__.return_value
            )
    ...
    async def test_type_string(self, monkeypatch, vnc_hid):
        with monkeypatch.context() as m:
            ...
            client_mock.keyboard.write.assert_has_calls(calls, any_order=False)
            client_mock.mouse.move.assert_called_once()
