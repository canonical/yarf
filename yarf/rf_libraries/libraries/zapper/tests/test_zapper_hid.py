...
from yarf.rf_libraries.libraries.zapper import ZapperException
from yarf.rf_libraries.libraries.zapper.Hid import Hid


@pytest.fixture(autouse=True)
def mock_zap():
    with patch("yarf.rf_libraries.libraries.zapper.Hid.zapper_api") as mock:
        yield mock
...
class TestZapperHid:
    ...
    async def test_walk_pointer_reuses_service(self, zapper_hid, mock_zap):
        ...
        mock_zap.reset_mock()
        with patch(
            "yarf.rf_libraries.libraries.hid_base.HidBase.walk_pointer_to_proportional",
            multi_move,
        ):
