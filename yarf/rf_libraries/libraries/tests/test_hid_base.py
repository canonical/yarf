...
class StubHid(HidBase):
    async def pointer_click_button(self, *args):
        pass
    ...
    async def _pointer_move(self, *args):
        pass

    async def pointer_press_button(self, *args):
        pass

    async def pointer_release_button(self, *args):
        pass

    async def pointer_release_buttons(self, *args):
        pass
...
class TestHidBase:
    @pytest.mark.asyncio
    async def test_pointer_move_to_absolute(self, stub_hid):
        """
        Test the mouse movement processing.
        """
        await stub_hid.pointer_move_to_absolute(100, 200)
        stub_hid._pointer_move.assert_called_with(
            100 / 1000,
            200 / 1000,
        )

    @pytest.mark.asyncio
    async def test_pointer_move_to_absolute_raises(self, stub_hid):
        """
        Test the mouse movement processing.
        """
        with pytest.raises(AssertionError):
            await stub_hid.pointer_move_to_absolute(1001, 0)

        with pytest.raises(AssertionError):
            await stub_hid.pointer_move_to_absolute(0, 1001)

    @pytest.mark.asyncio
    async def test_pointer_move_to_proportional(self, stub_hid):
        """
        Test the proportional mouse movement processing.
        """
        await stub_hid.pointer_move_to_proportional(0.5, 0.5)
        stub_hid._pointer_move.assert_called_with(0.5, 0.5)

    @pytest.mark.asyncio
    async def test_pointer_move_to_proportional_raises(self, stub_hid):
        """
        Test the function raises an exception if the target position is out of
        screen.
        """
        with pytest.raises(AssertionError):
            await stub_hid.pointer_move_to_proportional(1.1, 0)

        with pytest.raises(AssertionError):
            await stub_hid.pointer_move_to_proportional(0, 1.1)

    @pytest.mark.asyncio
    async def test_pointer_walk_to_proportional(self, stub_hid, mock_sleep):
        """
        Test the function moves the pointer by the requested step to the target
        position given in proportional coordinates.
        """
        await stub_hid.pointer_move_to_proportional(0.15, 0.35)
        await stub_hid.pointer_walk_to_proportional(0.5, 0.5, 0.05, 0.2)

        stub_hid._pointer_move.assert_has_calls(
            (
    ...
    @pytest.mark.asyncio
    async def test_pointer_walk_to_absolute(self, stub_hid, mock_sleep):
        """
        Test the function moves the pointer by the requested step to the target
        position given in absolute coordinates.
        """
        await stub_hid.pointer_move_to_proportional(0.15, 0.35)
        await stub_hid.pointer_walk_to_absolute(100, 200, 0.05, 0.2)
