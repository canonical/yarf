...
class TestMain:
    def test_parse_arguments(self) -> None:
        ...
        SUPPORTED_PLATFORMS["Zapper"] = Zapper
        argv = ["--platform", "Zapper", "suite-path"]
        args = parse_arguments(argv)
        assert args.platform == "Zapper"
        assert args.suite == "suite-path"
    ...
    def test_parse_arguments_system_argv(self) -> None:
        ...
        with patch.object(
            sys,
            "argv",
            ["prog", "--debug", "--platform", "Zapper", "suite-path"],
        ):
    ...
    def test_run_robot_suite(
        ...
        run_robot_suite(
            mock_test_suite, SUPPORTED_PLATFORMS["Zapper"], variables, outdir
        )
    ...
    def test_main_custom_outdir(
        ...
        mock_test_suite.assert_called_once()
        main.run_robot_suite.assert_called_once_with(
            mock_test_suite(), SUPPORTED_PLATFORMS["Zapper"], [], Path(outdir)
        )
